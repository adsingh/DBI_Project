...
QueryPlanNode* CreateQueryPlan(){
	...
	if(groupingAtts != NULL){
		...
		bool isDistinctGroupBy = distinctFunc == 1 ? true : false;
        // Create GroupBy Node
        currentNode->next = new GroupByNode(currentNode->outPipeID , pipeID, outSchema, grp_order, grpByAggFunction, isDistinctGroupBy);
		QueryPlanNode::numPipes = pipeID++;
		currentNode = currentNode->next;
		//currentNode->Print();
    ...
    // computeMe -- Use FuncOperator to generate this with the help of the Function::GrowFromParseTree
		...
		bool isDistinctSum = distinctFunc == 1 ? true : false;
        sumAggFunction->GrowFromParseTree (finalFunction, *currentNode->outSchema);
		Attribute* atts = new Attribute[1];
		atts[0].name = "sumResult";
		atts[0].myType = Double;

		currentNode->next = new SumNode(currentNode->outPipeID, pipeID, new Schema("sum_schema", 1, atts), sumAggFunction ,isDistinctSum);
		QueryPlanNode::numPipes = pipeID++;
		currentNode = currentNode->next;
		//currentNode->Print();
