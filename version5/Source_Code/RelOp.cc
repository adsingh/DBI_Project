...
void * Sum::workHandler(void * args){
	...
	while(worker_args->inPipe->Remove(&rec)){
		...
		if(resultType == Int){
			if(worker_args->isDistinctSum){
				intResults.insert(intResult);
			}	
			else
				result += intResult;
		}
			...
			if(worker_args->isDistinctSum){
				doubleResults.insert(doubleResult);
			}
			else
				result += doubleResult;
		}
...
void * GroupBy::workHandler(void * args){
	...
	while(!(pipeEmpty = sortedOutputPipe.Remove(&currRecord)==0) || !processingFinished){
		...
		if(!pipeEmpty && comp.Compare(&currRecord, &prevRecord, worker_args->orderMaker) == 0){
			...
			switch(resType){
				case Int:
					if(worker_args->isDistinctGroupBy){
						intResults.insert(res1);
					}
					else{
						runningIntSum += res1;
					}
			...
			if(resType == Int){
				...
				if(worker_args->isDistinctGroupBy){
					runningIntSum = 0;
					for(int intRes : intResults){
						runningIntSum += intRes;
					}
				}
				...
				if(worker_args->isDistinctGroupBy){
					runningDoubleSum = 0;
					for(int doubleRes : doubleResults){
						runningDoubleSum += doubleRes;
					}
				}
