...
void q4()
{
	...
	CNF cnf_p_ps;
	Record lit_p_ps;
	// get_cnf("(s_suppkey < 50)AND (ps_suppkey < 80)", s->schema(), ps->schema(), cnf_p_ps, lit_p_ps);
	...
	// int cnt = clear_pipe(_s_ps, &sum_sch, true);
...
void q6()
{
	...
	// ********** CUSTOM - Creating group order maker ***************
	//Attribute attr_grp[] = {s_nationkey};
	// Schema grpingSchema("grpSchem",1, attr_grp);
	CNF tempCnf;
	Record tempRecord;
...
*/
	...
	Attribute joinatt[] = {IA, SA, SA, s_nationkey, SA, DA, SA, ps_partkey, IA, IA, ps_supplycost, SA};
	Schema join_sch("join_sch", outAtts, joinatt);
	...
	CNF cnf_s_ps_p;
	Record lit_p_ps_s;
	get_cnf("(ps_partkey = p_partkey)", &join_sch, p->schema(), cnf_s_ps_p, lit_p_ps_s);
	int outAtts2 = outAtts + pAtts;
	Attribute joinatt2[] = {IA, SA, SA, s_nationkey, SA, DA, SA, ps_partkey, IA, IA, ps_supplycost, SA, IA, SA, SA, SA, SA, IA, SA, DA, SA};
	Schema join_sch2("join_sch2", outAtts2, joinatt2);

	J2.Run(_s_ps, _p, _s_ps_p, cnf_s_ps_p, lit_p_ps_s);
...
possible plan:
	...
	On _l:
		P (l_orderkey,l_partkey,l_suppkey) => __l  1, 2,3
	On __l:
		W (__l)
*/
	char *pred_l = "(l_returnflag = 'R')AND(l_discount < 0.04 OR l_shipmode = 'MAIL')";
	init_SF_li(pred_l, 100);
	SF_li.Run(dbf_li, _li, cnf_li, lit_li); // 10k recs qualified
	...
	Attribute atts[] = {IA, IA, IA};
	Schema outSch("outsch", 3, atts);

	P_li.Run(_li, _ps, keepMe, numAttsIn, numAttsOut);

	int cnt = clear_pipe(_ps, &outSch, false);

	SF_li.WaitUntilDone();
	P_li.WaitUntilDone();

	cout << " query8 returned " << cnt << " recs \n";
}
